#!/usr/bin/env python3
"""Exact rational lower-bound certificate for a 8-light square configuration.

For the literal decimal coordinates below, this program proves
    min_{p in [0,1]^2} sum_i 1/||p-c_i||^2 >= 47.8390.
...
PARAMETERS={
    "a":Q("0.11901099543098309"),
    "b":Q("0.15730761919588385"),
}

def build_lights()->list[Point]:
    a=PARAMETERS["a"]; b=PARAMETERS["b"]
    L=[
        (a,a),(ONE-a,a),(a,ONE-a),(ONE-a,ONE-a),
        (HALF,b),(HALF,ONE-b),(b,HALF),(ONE-b,HALF),
    ]
    assert len(L)==8 and len(set(L))==8
    assert set(L)=={(ONE-x,y) for x,y in L}
    assert set(L)=={(x,ONE-y) for x,y in L}
    assert set(L)=={(y,x) for x,y in L}
    return L
LIGHTS=build_lights()
